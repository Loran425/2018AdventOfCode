...
def main():
    ...
    for event in event_log:
        # Check for a guard ID ('#' followed by any number of digits)
        if re.search("#[0-9]+", event[1]):
    ...
    print(
        f"Target 1: Guard #{target_guard} @ minute {target_minute}, {target_guard*target_minute}\n"
    )
    ...
    print(
        f"Target 2: Guard #{target} @ minute {target_asleep[target][0]}, {target*target_asleep[target][0]}"
    )
